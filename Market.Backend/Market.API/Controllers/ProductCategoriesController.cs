...
public class ProductCategoriesController(ISender sender) : ControllerBase
{
    ...
    {
        string id = await sender.Send(command, ct);

        return CreatedAtAction(nameof(GetById), new { id }, new { id });
    }

    [HttpPut("{id:string}")]
    public async Task Update(string id, UpdateProductCategoryCommand command, CancellationToken ct)
    {
    ...
    [HttpGet("{id:string}")]
    public async Task<GetProductCategoryByIdQueryDto> GetById(string id, CancellationToken ct)
    {
